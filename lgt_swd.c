...
void lgt_set_delay_us(uint32_t us) { g_delay_us = us; }
static inline void swd_delay(void) { furi_delay_us(g_delay_us); }

/* ---- Pin-Primitive ---- */
static inline void swc_lo(void) { furi_hal_gpio_write(PIN_SWC, false); }
static inline void swc_hi(void) { furi_hal_gpio_write(PIN_SWC, true); }
static inline void swd_lo(void) { furi_hal_gpio_write(PIN_SWD, false); }
static inline void swd_hi(void) { furi_hal_gpio_write(PIN_SWD, true); }
static inline void rstn_lo(void){ furi_hal_gpio_write(PIN_RSTN, false); }
static inline void rstn_hi(void){ furi_hal_gpio_write(PIN_RSTN, true); }
static inline void swd_out(void){ furi_hal_gpio_init(PIN_SWD, GpioModeOutputPushPull, GpioPullNo, GpioSpeedVeryHigh); }
static inline void swd_in(void) { furi_hal_gpio_init(PIN_SWD, GpioModeInput, GpioPullNo, GpioSpeedVeryHigh); }
...
static void SWD_WriteByte(uint8_t start, uint8_t data, uint8_t stop) {
    int i;
    swd_out();
    if(start) { swc_lo(); swd_delay(); swd_lo(); swd_delay(); swc_hi(); swd_delay(); }
    for(i = 0; i < 8; i++) {
        swc_lo();
        if(data & 1) swd_hi(); else swd_lo();
        swd_delay();
        data >>= 1;
        swc_hi(); swd_delay();
    }
    swc_lo();
    if(stop) swd_hi(); else swd_lo();
    swd_delay();
    swc_hi(); swd_delay();
}
...
static void SWD_Idle(uint8_t cnt) {
    int i;
    swd_out(); swd_hi();
    for(i = 0; i < cnt; i++) { swc_lo(); swd_delay(); swc_hi(); swd_delay(); }
}

static void SWD_init(void) { swd_out(); swc_hi(); swd_hi(); }

static void SWD_ReadSWDID(uint8_t id[4]) {
    SWD_WriteByte(1, 0xAE, 1); SWD_Idle(4);
    swd_read(id, 4);
    SWD_Idle(4);
}
...
static void SWD_SWDEN(void) {
    SWD_WriteByte(1, 0xD0, 0); SWD_WriteByte(0, 0xAA, 0); SWD_WriteByte(0, 0x55, 0);
    SWD_WriteByte(0, 0xAA, 0); SWD_WriteByte(0, 0x55, 1); SWD_Idle(4);
}
static void SWD_UnLock0(void){ SWD_WriteByte(1,0xF0,0);SWD_WriteByte(0,0x54,0);SWD_WriteByte(0,0x51,0);SWD_WriteByte(0,0x4A,0);SWD_WriteByte(0,0x4C,1);SWD_Idle(4);}
static void SWD_UnLock1(void){ SWD_WriteByte(1,0xF0,0);SWD_WriteByte(0,0x00,0);SWD_WriteByte(0,0x00,0);SWD_WriteByte(0,0x00,0);SWD_WriteByte(0,0x00,1);SWD_Idle(4);}
static void SWD_UnLock2(void){ SWD_WriteByte(1,0xF0,0);SWD_WriteByte(0,0x43,0);SWD_WriteByte(0,0x40,0);SWD_WriteByte(0,0x59,0);SWD_WriteByte(0,0x5D,1);SWD_Idle(4);}
...
static void SWD_EEE_Write(uint32_t data, uint16_t addr) {
    SWD_EEE_DSEQ(data);
    SWD_EEE_CSEQ(0x86, addr); SWD_EEE_CSEQ(0xc6, addr); SWD_EEE_CSEQ(0x86, addr);
}
static uint32_t SWD_EEE_Read(uint16_t addr) {
    uint8_t d[4];
    SWD_EEE_CSEQ(0xc0, addr); SWD_EEE_CSEQ(0xe0, addr);
...
static void lgt_write_page(uint32_t byteAddr, const uint8_t* buf, int size) {
    int i; uint16_t a = (uint16_t)(byteAddr / 4);   /* 4-Byte-Wortadresse */
    SWD_EEE_CSEQ(0x00, a); SWD_EEE_CSEQ(0x84, a); SWD_EEE_CSEQ(0x86, a);
    for(i = 0; i < size; i += 4) {
        uint32_t w = (uint32_t)buf[i] | ((uint32_t)buf[i+1]<<8) | ((uint32_t)buf[i+2]<<16) | ((uint32_t)buf[i+3]<<24);
        SWD_EEE_Write(w, a);
        a++;
    }
    SWD_EEE_CSEQ(0x82, (uint16_t)(a-1)); SWD_EEE_CSEQ(0x80, (uint16_t)(a-1)); SWD_EEE_CSEQ(0x00, (uint16_t)(a-1));
}
static void lgt_read_page(uint32_t byteAddr, uint8_t* buf, int size) {
    int i; uint16_t a = (uint16_t)(byteAddr / 4); uint32_t data = 0;
    SWD_EEE_CSEQ(0x00, 0x01);
    for(i = 0; i < size; i++) {
        if(i % 4 == 0) { data = SWD_EEE_Read(a); a++; }
...
static uint8_t SWD_UnLock(uint8_t chip_erase) {
    ...
    if(swdid[0] == 0x3e) {
        ...
        SWD_WriteByte(0,0x0c,0);SWD_WriteByte(0,0x00,0);SWD_WriteByte(0,0x0f,1);SWD_Idle(40);
        SWD_UnLock2();
    }
    SWD_Idle(40);
    SWD_WriteByte(1,0xb1,0);SWD_WriteByte(0,0x0c,0);SWD_WriteByte(0,0x00,0);SWD_WriteByte(0,0x17,1);SWD_Idle(40);
    SWD_WriteByte(1,0xa9,1); SWD_Idle(4);
    swd_read(flag, 2);
    SWD_Idle(4);
    if(flag[1] == 0x20) {
        SWD_WriteByte(1,0xb1,0);SWD_WriteByte(0,0x3d,0);SWD_WriteByte(0,0x20,0);
        SWD_WriteByte(0,0x0c,0);SWD_WriteByte(0,0x00,0);SWD_WriteByte(0,0x0f,1);SWD_Idle(40);
    } else if(flag[1] == 0x60) {
        /* ok (RESET-Pin-Variante) */
    } else return 0;
    SWD_WriteByte(1,0xb1,0);SWD_WriteByte(0,0x0d,1);SWD_Idle(2);
    return 1;
}
static void SWD_exit(void) {
    SWD_WriteByte(1,0xb1,0); SWD_WriteByte(0,0x0d,1); SWD_Idle(2); furi_delay_ms(1);
    SWD_WriteByte(1,0xb1,0); SWD_WriteByte(0,0x0c,1); SWD_Idle(2); SWD_Idle(40);
}
...
static void end_pmode(void) {
    SWD_exit();
    rstn_hi();
    furi_hal_gpio_init(PIN_RSTN, GpioModeInput, GpioPullNo, GpioSpeedLow);  /* RESET frei -> Target laeuft */
}

/* ============ High-Level ============ */
static int page_is_empty(const uint8_t* img, uint32_t base) {
    for(uint32_t i = 0; i < LGT_PAGE_BYTES; i++) if(img[base + i] != 0xFF) return 0;
    return 1;
}
...
int lgt_flash(const uint8_t* img, uint32_t img_len, bool verify, LgtProgressCb cb, void* ctx) {
    ...
    if(verify) {
        for(a = 0; a < img_len; a += LGT_PAGE_BYTES) {
            uint8_t rb[LGT_PAGE_BYTES]; uint32_t j;
...
int lgt_verify(const uint8_t* img, uint32_t img_len, LgtProgressCb cb, void* ctx) {
    uint32_t a; int bad = 0;
    lgt_gpio_init();
    if(!start_pmode(1)) { end_pmode(); lgt_gpio_deinit(); return -1; }  /* Unlock loescht -> Verify nur direkt nach Write sinnvoll */
    for(a = 0; a < img_len; a += LGT_PAGE_BYTES) {
        uint8_t rb[LGT_PAGE_BYTES]; uint32_t j;
        uint32_t nn = (img_len - a >= LGT_PAGE_BYTES) ? LGT_PAGE_BYTES : (img_len - a);
        if(page_is_empty(img, a)) continue;
