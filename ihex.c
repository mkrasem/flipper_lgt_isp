...
static int b2(const char* s, int i) { return (hx(s[i]) << 4) | hx(s[i + 1]); }
...
int ihex_parse(const char* text, size_t len, uint8_t* img, uint32_t img_cap, uint32_t* out_len) {
    ...
    while(p < len) {
        ...
        if(type == 0x04) { base = ((uint32_t)((b2(line, 9) << 8) | b2(line, 11))) << 16; continue; }
...
int ihex_record(char* out, size_t cap, uint16_t addr, const uint8_t* data, uint8_t n) {
    ...
    for(int i = 0; i < n; i++) { off += snprintf(out + off, cap - off, "%02X", data[i]); sum += data[i]; }
